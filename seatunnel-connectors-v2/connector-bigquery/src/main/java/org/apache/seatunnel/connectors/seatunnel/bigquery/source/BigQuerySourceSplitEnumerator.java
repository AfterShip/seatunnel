...
import com.google.cloud.hadoop.io.bigquery.*;
 ...
 * @date 2023/8/7 17:46
 */
public class BigQuerySourceSplitEnumerator implements SourceSplitEnumerator<BigQuerySourceSplit, BigQuerySourceState> {
     ...
     * @throws InterruptedException if the job is interrupted while waiting for the query to complete.
     */
    private void runJob() throws IOException, InterruptedException {
        log.info("Run query: {}", sql);
        TableReference sourceTable = new TableReference().setDatasetId(datasetId).setProjectId(projectId).setTableId(tableName);
        String location = bigQueryHelper.getTable(sourceTable).getLocation();
        TableReference exportTableReference = createExportTableReference(projectId, datasetId, temporaryTableName);
     ...
     */
    private TableReference createExportTableReference(String datasetProjectId, String datasetId, String tableId) {
    ...
    private static void runQuery(
        ...
        com.google.api.services.bigquery.model.Job response = bigQueryHelper.insertJobOrFetchDuplicate(projectId, job);
     ...
     */
    private static JobReference getJobReference(BigQueryHelper bigQueryHelper, String projectId, String location) {
    ...
    private List<BigQuerySourceSplit> getBigQuerySplit() throws IOException, InterruptedException {
        ...
        inputSplits.forEach(x -> splits.add(new BigQuerySourceSplit(String.valueOf(x.hashCode()), (UnshardedInputSplit) x)));
        return splits;
    }
